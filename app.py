...
def to_display_rgb(color):
    """
    Convert sampled pixel color to the effective RGB shown on a white canvas.
    Handles RGBA pixels from partially transparent backgrounds.
    """
    if len(color) >= 4:
        r, g, b, a = [int(x) for x in color[:4]]
        alpha = a / 255.0
        return (
